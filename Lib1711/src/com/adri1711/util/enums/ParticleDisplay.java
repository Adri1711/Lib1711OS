...
/**
 * By default the particle xyz offsets and speed aren't 0, but everything will
 * be 0 by default in this class. Particles are spawned to a location. So all
 * the nearby players can see it.
 * <p>
 * The fields of this class are publicly accessible for ease of use. All the
 * fields can be null except the particle type.
 * <p>
 * For cross-version compatibility, instead of Bukkit's {@link org.bukkit.Color}
 * the java awt {@link Color} class is used.
 * <p>
 * the data field is used to store special particle data, such as colored
 * particles. For colored particles a float list is used since the particle size
 * is a float. The format of float list data for a colored particle is:
...
public class ParticleDisplay implements Cloneable {
	...
	private static final Particle1711 DEFAULT_PARTICLE = Particle1711.CLOUD;

	@Nonnull
	public Particle1711 particle;
	@Nullable
	...
	/**
	 * Make a new instance of particle display. The position of each particle
	 * will be randomized positively and negatively by the offset parameters on
	 * each axis.
	 *
	 * @param particle
	 ...
	 */
	public ParticleDisplay(API1711 api, @Nonnull Particle1711 particle, @Nullable Callable<Location> locationCaller,
			@Nullable Location location, int count, double offsetx, double offsety, double offsetz, double extra) {
	...
	public ParticleDisplay(API1711 api, @Nonnull Particle1711 particle, @Nullable Location location, int count,
			double offsetx, double offsety, double offsetz) {
		this(api, particle, null, location, count, offsetx, offsety, offsetz, 0);
	}

	public ParticleDisplay(API1711 api, @Nonnull Particle1711 particle, @Nullable Location location, int count) {
		this(api, particle, location, count, 0, 0, 0);
	}

	public ParticleDisplay(API1711 api, @Nonnull Particle1711 particle, @Nullable Location location) {
		this(api, particle, location, 0);
	}

	/**
	 * Builds a simple ParticleDisplay object with cross-version compatible
	 * {@link org.bukkit.Particle.DustOptions} properties.
	 *
	 * @param location
	 ...
	 */
	@Nonnull
	public static ParticleDisplay colored(API1711 api, @Nullable Location location, int r, int g, int b, float size) {
		ParticleDisplay dust = new ParticleDisplay(api, Particle1711.REDSTONE, null, location, 1, 0, 0, 0, 0);
	 ...
	 */
	@Nonnull
	public static ParticleDisplay colored(API1711 api, @Nullable Location location, @Nonnull Color color, float size) {
		return colored(api, location, color.getRed(), color.getGreen(), color.getBlue(), size);
	}

	/**
	 * Builds a simple ParticleDisplay object. An invocation of this method
	 * yields exactly the same result as the expression:
	 * <p>
	 * <blockquote> new ParticleDisplay(particle, location, 1, 0, 0, 0, 0);
	 * </blockquote>
	 *
	 * @param location
	 *            the location of the display.
	 * @param particle
	 *            the particle of the display.
	 * @return a simple ParticleDisplay.
	 * @since 1.0.0
	 */
	@Nonnull
	public static ParticleDisplay simple(API1711 api, @Nullable Location location, @Nonnull Particle1711 particle) {
		Objects.requireNonNull(particle, "Cannot build ParticleDisplay with null particle");
		return new ParticleDisplay(api, particle, null, location, 1, 0, 0, 0, 0);
	}

	/**
	 * A quick access method to display a simple particle. An invocation of this
	 * method yields the same result as the expression:
	 * <p>
	 * <blockquote> ParticleDisplay.simple(location, particle).spawn();
	 * </blockquote>
	 *
	 * @param location
	 ...
	 */
	@Nonnull
	public static ParticleDisplay display(API1711 api, @Nonnull Location location, @Nonnull Particle1711 particle) {
		Objects.requireNonNull(location, "Cannot display particle in null location");
		ParticleDisplay display = simple(api, location, particle);
		display.spawn();
		return display;
	}
	 ...
	 *
	 * @param location
	 *            the location to display this particle.
	 * @param config
	 *            the config section for the settings.
	 * @return a parsed ParticleDisplay.
	 * @since 1.0.0
	 */
	public static ParticleDisplay fromConfig(API1711 api, @Nullable Location location,
			@Nonnull ConfigurationSection config) {
		ParticleDisplay display = new ParticleDisplay(api, DEFAULT_PARTICLE, location);
	 ...
	 *
	 * @param display
	 *            the particle display settings to update.
	 * @param config
	 *            the config section for the settings.
	...
	public static ParticleDisplay edit(@Nonnull ParticleDisplay display, @Nonnull ConfigurationSection config) {
		...
		Particle1711 particle = particleName == null ? null : XParticle.getParticle(particleName);
	...
	/**
	 * Rotates the given xyz with the given rotation radians and adds the to the
	 * specified location.
	 *
	 * @param location
	 *            the location to add the rotated axis.
	 * @param rotation
	 *            the xyz rotation radians.
	...
	/**
	 * We don't want to use {@link Location#clone()} since it doens't copy to
	 * constructor and Javas clone method is known to be inefficient and broken.
	 *
	 * @since 3.0.3
	 */
	@Nonnull
	private static Location cloneLocation(@Nonnull Location location) {
		return new Location(location.getWorld(), location.getX(), location.getY(), location.getZ(), location.getYaw(),
				location.getPitch());
	}

	@Override
	public String toString() {
		return "ParticleDisplay:[Particle=" + particle + ", Count=" + count + ", Offset:{" + offsetx + ", " + offsety
				+ ", " + offsetz + "}, Extra=" + extra + ", Data="
				+ (data == null ? "null" : data instanceof float[] ? Arrays.toString((float[]) data) : data);
	}
	 ...
	 *
	 * @param entity
	 *            the entity to track the location from.
	 ...
	 *            the caller to call to get the new location.
	...
	/**
	 * Adjusts the rotation settings to face the entitys direction. Only some of
	 * the shapes support this method.
	 *
	 * @param entity
	 *            the entity to face.
	 * @return the same particle display.
	 ...
	 *
	 * @param x
	 *            the x to add to the location.
	 * @param y
	 *            the y to add to the location.
	 * @param z
	 *            the z to add to the location.
	...
	public ParticleDisplay clone() {
		ParticleDisplay display = new ParticleDisplay(api, particle, locationCaller,
				(location == null ? null : cloneLocation(location)), count, offsetx, offsety, offsetz, extra);
		if (rotation != null)
			display.rotation = new Vector(rotation.getX(), rotation.getY(), rotation.getZ());
		display.data = data;
		return display;
	}
	 ...
	 *
	 * @param vector
	 *            the vector to rotate from. The xyz values of this vectors must
	 *            be radians.
	...
	/**
	 * Rotates the particle position based on the xyz radians. Rotations are
	 * only supported for some shapes in {@link XParticle}. Rotating some of
	 * them can result in weird shapes.
	 *
	 * @see #rotate(Vector)
	 * @since 3.0.0
	 */
	...
	/**
	 * When a particle is set to be directional it'll only spawn one particle
	 * and the xyz offset values are used for the direction of the particle.
	 * <p>
	...
	/**
	 * Adds xyz of the given vector to the cloned location before spawning
	 * particles.
	 *
	 * @param location
	 *            the xyz to add.
	...
	/**
	 * Displays the particle in the specified location. This method does not
	 * support rotations if used directly.
	 *
	 * @param loc
	 ...
	 *            the location to display the particle at.
	 * @param players
	 *            if this particle should only be sent to specific players.
	 * @see #spawn(double, double, double)
	 * @since 5.0.0
	 */
	@Nonnull
	public Location spawn(@Nonnull Location loc, @Nullable Player... players) {
		if (data != null) {
			if (data instanceof float[]) {
				...
				if (ISFLAT) {
					Particle.DustOptions dust = new Particle.DustOptions(
							org.bukkit.Color.fromRGB((int) datas[0], (int) datas[1], (int) datas[2]), datas[3]);
					if (players == null)
						api.spawnParticle(loc.getWorld(), particle, loc, count, offsetx, offsety, offsetz, extra, dust);
					else
						for (Player player : players)
							api.spawnParticle(player.getLocation().getWorld(), particle, loc, count, offsetx, offsety,
									offsetz, extra, dust);
				} else {
					if (players == null)
						api.spawnParticle(loc.getWorld(), particle, loc, count, (int) datas[0], (int) datas[1],
								(int) datas[2], datas[3], null);
					else
						for (Player player : players)
							api.spawnParticle(player.getLocation().getWorld(), particle, loc, count, (int) datas[0],
									(int) datas[1], (int) datas[2], datas[3], null);
				}
			}
		} else {
			SpigotVersion version = SpigotVersion.getValueOf(api.getsVersion());
			...
			if (version != null) {
				...
				case V1_12:
					if (players == null)
						api.spawnParticle(loc.getWorld(), particle, loc, count, offsetx, offsety, offsetz, extra, null);
					else
						for (Player player : players)
							api.spawnParticle(player.getLocation().getWorld(), particle, loc, count, offsetx, offsety,
									offsetz, extra, null);
				...
				default:
					...
					if (particle.equals(Particle1711.REDSTONE)) {
						dust = XParticle.randomDust();
					}
					if (players == null)
						api.spawnParticle(loc.getWorld(), particle, loc, count, offsetx, offsety, offsetz, extra, dust);
					else
